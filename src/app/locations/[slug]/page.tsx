...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `${loc.blurb} GL, contractor pollution liability, workers' comp, commercial auto, and tools & equipment for ${loc.name} spray contractors. 15-minute quotes.`,
  ...
  return (
    <>
      ...
      <main>
            ...
            <div className="grid lg:grid-cols-12 gap-10 items-center">
              ...
              <FadeIn direction="left" className="lg:col-span-5">
                <div className="rounded-t-[10rem] rounded-b-3xl overflow-hidden border-4 border-white shadow-warm-lg">
                  <img src="/images/coverage.jpg" alt={`Spray contractor in ${loc.name}`} className="w-full h-[300px] md:h-[360px] object-cover" loading="lazy" />
                </div>
              </FadeIn>
            </div>
          </div>
        ...
        <section className="bg-cream py-16 md:py-20">
          <div className="container-tight">
            <FadeIn className="max-w-2xl mb-10">
              <span className="eyebrow"><span className="h-px w-8 bg-clay" />Coverage for {loc.name} spray contractors</span>
              <h2 className="mt-3 h-section">The full program, built for {loc.name} spray contractors.</h2>
              <p className="mt-4 lead">From a solo applicator to a multi-crew spray operation, we coordinate every line a {loc.name} spray contractor needs.</p>
            </FadeIn>
        ...
        <FAQ items={locFaqs} eyebrow={`${loc.name} — FAQ`} title={<>Spray contractor insurance questions for <span className="text-clay">{loc.name}</span></>} background="sand" />

        <CTABand title={`Insuring spray contractors in ${loc.name} since 2005`} description={`Specialty markets, proper spray trade class codes, and 15-minute quotes. Call ${SITE.phone} or request a quote online.`} />
      </main>
      <Footer />
    </>
  );
